...
Si las dos corridas (la nativa de GitHub y la de cron-job.org) llegan a
dispararse casi al mismo tiempo, el workflow tiene un paso llamado **"Reservar
turno"** que hace que solo una de las dos siga adelante: la que pierde esa
carrera se cancela sola, antes de publicar nada en YouTube o Instagram. Vas a
ver ese paso en los logs de cada corrida — es normal, y en la gran mayoría de
los casos no hace nada más que confirmar "turno reservado" en un segundo. (Esto
reemplaza una protección más simple que había antes y que, en la práctica,
falló una vez y causó que un video se publicara dos veces al feed normal;
quedó documentado en el proyecto como Incidente 8.)
